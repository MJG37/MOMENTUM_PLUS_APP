...
type Reward = { id: string; name: string; description: string; cost: number; icon: keyof typeof Ionicons.glyphMap; color: string };
...
export default function RewardsScreen() {
  ...
  const saveCustomReward = async () => {
    ...
    try {
      await addCustomReward({ owner: username ?? "", name: rewardName, description: rewardDescription, cost });
      setRewardName(""); setRewardDescription(""); setRewardCost(""); setIsAddingReward(false);
    } catch {
      Alert.alert("Could not save reward", "Please check your connection and try again.");
    } finally { setIsSavingReward(false); }
  };

  const rewards: Reward[] = [...REWARDS, ...(customRewards ?? []).map((reward) => ({ id: reward._id, name: reward.name, description: reward.description, cost: reward.cost, icon: "sparkles" as const, color: "#14b8a6" }))];
  ...
  return <LinearGradient colors={colors.gradients.background} style={styles.container}>
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.rewardsHeading}><Text style={styles.sectionTitle}>Rewards</Text><TouchableOpacity style={[styles.addRewardButton, { borderColor: colors.primary }]} onPress={() => setIsAddingReward(true)}><Ionicons name="add" size={18} color={colors.primary} /><Text style={[styles.addRewardText, { color: colors.primary }]}>Add your own</Text></TouchableOpacity></View>
